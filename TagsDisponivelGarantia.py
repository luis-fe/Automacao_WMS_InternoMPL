...
def BuscarTagsGarantia():
    ...
    consulta = pd.read_sql('SELECT '
                           'p.codBarrasTag as codbarrastag , p.codReduzido as codreduzido, p.codEngenharia as engenharia,'
                        ...
                        ' p.numeroOP in ( SELECT numeroOP  FROM tco.OrdemProd o WHERE codEmpresa = 1 and codFaseAtual in (210, 320, 56, 432, 441 ) and situacao = 3) '
                            '', conn)
    conn.close()
    return consulta
