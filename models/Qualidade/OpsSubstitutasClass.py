...
class Ops_Substitutas():
    ...
    def substitutosSkuOP(self):
        ...
        consultaSortVar['tipo'] = 'Variavel' #2.2.1 - Acrescentando ao DataFrame a coluna Tipo (identificando que trata-se de componentes variaveis.
        ...
        consultaSubstitudos = pd.merge(consultaSubstitudos, consultaSortVar, on=['codproduto', 'componente'],
                                       how='left')
        consultaSubstitudos['tipo'].fillna('Padrao', inplace=True)  # 3.1 Caso no merge nao identifique o tipo Variavel, substitui a informacao para tipo "Padrao" que será reanalizado junto ao DataFrame consultaPad.
        ...
        consulta['categoria'] = consulta.apply(
            lambda row: self.__categoria('KIT GOLA', row['nomecompontente'], 'KIT GOLA/PUNHO', row['categoria']), axis=1)
        consulta['categoria'] = consulta.apply(
        ...
        consulta['categoria'] = consulta.apply(
            lambda row: self.__categoria('BOTAO DE POLIESTER', row['nomecompontente'], 'RETIRAR', row['categoria']), axis=1)
        ...
        consulta['categoria'] = consulta.apply(
            lambda row: self.__categoria('ETIQUETA DE TAMANHO', row['nomecompontente'], 'RETIRAR', row['categoria']), axis=1)

        consulta['categoria'] = consulta.apply(
            lambda row: self.__categoria('ETIQUETA TAMANHO', row['nomecompontente'], 'RETIRAR', row['categoria']), axis=1)
        ...
        consulta['categoria'] = consulta.apply(
            lambda row: self.__categoria('COLARINHO PARA CAMISA', row['nomecompontente'], 'RETIRAR', row['categoria']), axis=1)
